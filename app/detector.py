...
class WildlifeDetector:
    ...
    def detect_and_crop(self, frame_bgr: cv2.Mat, timestamp: float = 0.0) -> List[Dict[str, Any]]:
        """
        Runs MegaDetector v5a on a single frame, crops detected objects,
        saves the cropped images locally in folders categorized by category,
        ...
        for _, row in detections_df.iterrows():
            ...
            if success:
                ...
                relative_path = f"detections/{category}/{crop_filename}"
                # Static HTTP URL prefix
                crop_url = f"{DETECTIONS_URL_PREFIX}/{category}/{crop_filename}"
            else:
                ...
                try:
                    ...
                    if preprocessed is not None:
                        ...
                        if isinstance(result, dict):
                            # Attempt 1: Direct prediction and confidence keys
                            species = result.get("prediction")
                            species_conf = result.get("confidence") or result.get("probability") or result.get("score")
                            
                            # Attempt 2: Predictions list
                            predictions_list = result.get("predictions")
                            if not species and isinstance(predictions_list, list) and len(predictions_list) > 0:
                                top_pred = predictions_list[0]
                                if isinstance(top_pred, dict):
                                    species = top_pred.get("prediction") or top_pred.get("label") or top_pred.get("category")
                                    species_conf = top_pred.get("probability") or top_pred.get("score") or top_pred.get("confidence")
